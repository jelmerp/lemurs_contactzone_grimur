...
[[ $BARCODE_TABLE = "" ]] && echo "## ERROR: Please provide barcode file with -b flag" && exit 1
[[ $INDIR = "" ]] && echo "## ERROR: Please provide input dir with -i flag" && exit 1
[[ $LIBRARY_ID = "" ]] && echo "## ERROR: Please provide library ID with -l flag" && exit 1
[[ $OUTDIR = "" ]] && echo "## ERROR: Please provide output dir with -o flag" && exit 1


# OTHER SET-UP -----------------------------------------------------------------
## Scripts
SCRIPT_FLIP_PERL=scripts/fastq_process/flip_trim_sbfI_170601.pl
SCRIPT_FLIP_BASH=scripts/fastq_process/flipreads.sh
SCRIPT_DEMULT=scripts/fastq_process/demultiplex.sh
SCRIPT_DEDUP_TRIM=scripts/fastq_process/fqdeduptrim.sh
SCRIPT_CHECKBARCODES=scripts/fastq_process/checkbarcodes.sh
SCRIPT_QC_FASTQ=scripts/qc/qc_fastq.sh
SCRIPT_STATS_FASTQ=scripts/qc/qc_fastq_stats.sh

## Hardcoded variables
ADAPTER_FILE=software/Trimmomatic-0.36/adapters/all.fa    # For trimmomatic
...
mkdir -p "$DIR_FLIPPED" "$DIR_DEMULT" "$DIR_FINAL" "$DIR_STATS_IND"
...
echo -e "\n## Done with script 01_fastq-process_runner.sh"
date
echo
