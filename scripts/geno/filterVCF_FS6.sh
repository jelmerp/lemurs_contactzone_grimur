...
echo -e "\n#####################################################################"
date
echo "## filterVCF_FS6.sh: Starting script."
date
...
MAWK=software/mawk-1.3.4-20171017/mawk
...
[[ ! -e $VCF_IN ]] && [[ ! -e $VCF_IN.gz ]] && echo -e "\n\n## filterVCF_FS6.sh: ERROR: CANT FIND INPUT VCF FILE" && exit 1
...
then
	...
	echo -e "\n\n## filterVCF_FS6.sh: ERROR: NR OF INDS REQUESTED ($NR_INDS_REQUESTED) DOES NOT MATCH NR PRESENT ($NR_INDS_PRESENT) IN VCF\n" && exit 1 
