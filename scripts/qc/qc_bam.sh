...
[[ ! -f $INPUT.bai ]] && echo -e "## qc_bam.sh: Indexing bam file..." && $SAMTOOLS index -b $INPUT

echo -e "## qc_bam.sh: Bam input file:"
ls -lh $INPUT
...
################################################################################
## Depth/coverage:
echo -e "## qc_bam.sh: Checking coverage..."
...
## Samtools flagstat:
echo -e "## qc_bam.sh: Running samtools flagstat..."
$SAMTOOLS flagstat $INPUT > $OUTDIR/$ID.samtools-flagstat.txt
echo -e "## qc_bam.sh: samtools flagstat output file:..."
ls -lh $OUTDIR/$ID.samtools-flagstat.txt

## ea-utils samstats:
echo -e "## qc_bam.sh: Running ea-utils samstats..."
$EAUTILS_SAMSTATS -D -B $INPUT > $OUTDIR/$ID.ea-utils-samstats.txt
echo -e "## qc_bam.sh: ea-utils output file:..."
...
################################################################################
echo -e "## qc_bam.sh: Done with script qc_bam.sh"
date
echo
