...
p.dist <- ggmap(my.map, extent = 'device') +
  geom_polygon(aes(x = long, y = lat, group = id), data = shapedata_gri,
               fill = col.gri, alpha = 0.4, size = 0) +
  geom_polygon(aes(x = long, y = lat, group = id), data = shapedata_mur,
               fill = col.mur, alpha = 0.3, size = 0) +
  geom_point(data = locs, colour = 'black', size = 12, stroke = 6,
  ...
  scale_fill_manual(values = c('black', '#FFB90F', '#FF00FF'),
                    labels = c('hybridzone', 'griseorufus', 'murinus'), guide = FALSE) +
...
##### SAVE MAP ####
################################################################################
ggsave(outfile_map_eps, width = 20, height = 15,
