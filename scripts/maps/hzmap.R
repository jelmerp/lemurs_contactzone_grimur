...
outfile_map_msats_eps <- paste0(plotdir, '/HZmap_msats.eps')
outfile_map_radseq_eps <- paste0(plotdir, '/HZmap_radseq.eps')
...
sp.labs2 <- c(expression(italic("griseorufus")), expression(plain("    hybrid")),
              expression(italic("  murinus")))
hyb.labs <- c('hybrid', 'non-hybrid')
sp.cols <- c('#D69B12', '#FF00FF')
sp.cols2 <- c('#D69B12', 'grey20', '#FF00FF')
msat.name <- 'microsatellite\nassignment:'
radseq.name <- '\nRADseq\nassignment:'
...
p.Mtk <- ggmap(map.Mtk, extent = 'device') +
  geom_point(data = coords_Mtk, colour = 'black', size = 4, stroke = 1, shape = 21,
             aes(x = lon, y = lat, fill = species.short)) +
  ...
  scale_fill_manual(values = sp.cols2) +
...
p.Tsm <- ggmap(map.Tsm, extent = 'device') +
  geom_point(data = coords_Tsm, colour = 'black', size = 4, stroke = 1, shape = 21,
             aes(x = lon, y = lat, fill = species.short)) +
  ...
  scale_fill_manual(values = sp.cols2, labels = sp.labs2, name = msat.name) +
  guides(fill = guide_legend(override.aes = list(shape = 21), label.hjust = -1)) +
...
## Save map:
p <- ggarrange(p.Mtk, p.Tsm, ncol = 2, nrow = 1, widths = c(1, 1.673))
ggexport(p, filename = outfile_map_radseq_eps)
system(paste0('xdg-open ', outfile_map_radseq_eps))
