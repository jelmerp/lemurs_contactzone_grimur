...
plot_d <- function(
  ...
  marg_sig = FALSE,
  ylab = "D\nD<0: p1p3 admixture\nD>0: p2p3 admixture",
  draw_rect = FALSE, rect_coord = NULL,
  zero_line = TRUE, hline = NULL, hline2 = NULL,
  breaks_by = 0.05, pointsize = 1,
  axis.title.x.size = 16, axis.text.x.size = 16, axis.text.y.size = 14,
  figsave = FALSE, figfile = NULL, figdims = c(5, 5) # figdims: c(width, height)
  ) {
  ...
  if(marg_sig == TRUE) {
    d$sig <- cut(abs(d$Z), breaks = c(0, 2, 3, Inf), labels = c('grey10', 'orange', 'red'))
    d$siglab <- gsub('grey10', '|Z|<2', d$sig)
  ...
  cols.all <- c('grey10', 'orange', 'red', 'black')
  ...
  p <- p +
    geom_pointrange(aes(ymax = -D + se, ymin = -D - se, colour = sig),
                    size = pointsize) +
    labs(y = ylab) +
