...
library(cowplot)
library(here)
script_atools_fun <- here('scripts/genscriptlink_lemurs/admixtools/admixtools_plot_fun.R')
source(script_atools_fun)

## Output dirs and files:
plotdir <- 'analyses/admixtools/finalfigures/'
figfile_png <- paste0(plotdir, '/admixstats.png')
figfile_eps <- paste0(plotdir, '/admixstats.eps')
#figfile.d <- paste0(plotdir, '/Dstats_bySupersite2.png')
#figfile.d <- paste0(plotdir, '/f4ratio.png')


################################################################################
##### D-STATS BY SUPERSITE2 ####
################################################################################
## File ID: ## output should be: analyses/admixtools/output/$file.id.dmode.out
