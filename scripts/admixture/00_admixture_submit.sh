...
## Settings
VCF_DIR=results/geno/vcf/gatk/         # Dir with existing VCF file(s)
PLINK_DIR=results/conversions/plink    # Dir for PLINK files (to be produced)
...
for file_id in "${FILE_IDS[@]}"; do
    ...
    #sbatch --mem 8G -o slurm_admixture_runner_"$file_id" \
    scripts/admixture/01_admixture_runner.sh "$file_id" "$VCF_DIR" "$PLINK_DIR" "$ADMIX_OUTDIR" -k 1 -K 9
done
